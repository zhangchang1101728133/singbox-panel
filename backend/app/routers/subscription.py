...
async def add_subscription(sub: SubscriptionCreate, user: dict = Depends(get_current_user)):
    ...
    async with get_db() as db:
        ...
        cursor = await db.execute(
            ...
            (user["id"], sub.name, "", sub.slug, json.dumps(sub.node_ids))
        )
        sub_id = cursor.lastrowid
        token = f"sub_{sub_id}"
        await db.execute("UPDATE subscriptions SET token = ? WHERE id = ?", (token, sub_id))
        await db.commit()
...
async def delete_subscription(sub_id: int, user: dict = Depends(get_current_user)):
    """删除订阅"""
    async with get_db() as db:
        cursor = await db.execute("SELECT token FROM subscriptions WHERE id = ?", (sub_id,))
        sub = await cursor.fetchone()
        if not sub:
            raise HTTPException(status_code=404, detail="订阅不存在")
        sub_path = SingboxService.SUB_DIR / f"{sub['token']}.json"
...
async def download_subscription(name: str):
    """下载订阅配置 - 支持 token 或 slug 访问"""
    async with get_db() as db:
        cursor = await db.execute("SELECT token FROM subscriptions WHERE slug = ?", (name,))
        sub = await cursor.fetchone()
        if not sub:
            cursor = await db.execute("SELECT token FROM subscriptions WHERE token = ?", (name,))
            sub = await cursor.fetchone()
        if not sub:
            raise HTTPException(status_code=404, detail="订阅不存在")
...
async def _generate_sub_file(db, sub_id: int, node_ids: List[int]):
    ...
    SingboxService.save_subscription(client_config, f"sub_{sub_id}")
